...
export async function generateConceptDepth(
  ...
  // Cắt sau 20s: nếu Gemini treo hoặc mạng chậm thì rơi về dữ liệu nền thay vì để màn hình
  // kẹt mãi ở "Đang sinh AI…" — rủi ro thật khi demo live.
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      return { ok: false, error: 'Gemini quá 20s không trả lời — dùng dữ liệu nền.' }
    }
    return {
      ok: false,
