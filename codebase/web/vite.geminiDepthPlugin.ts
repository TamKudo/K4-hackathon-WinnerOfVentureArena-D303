...
/**
 * Đọc key theo thứ tự: codebase/web/.env → .env gốc repo → biến môi trường shell.
 * Nhận cả hai vị trí .env vì cả nhóm hay để key ở gốc repo (nơi GROQ_API_KEY đang nằm)
 * — trước đây chỉ đọc codebase/web/.env nên key ở gốc bị bỏ qua mà không báo gì.
 */
function getApiKey(mode: string): string {
  const local = loadEnv(mode, rootDir, '')
  if (local.GEMINI_API_KEY?.trim()) return local.GEMINI_API_KEY.trim()

  const repoRoot = path.resolve(rootDir, '../..')
  const atRoot = loadEnv(mode, repoRoot, '')
  if (atRoot.GEMINI_API_KEY?.trim()) return atRoot.GEMINI_API_KEY.trim()
...
function makeHandler(mode: string) {
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (!apiKey) {
      // 501 Not Implemented, không phải 503: chưa bật tính năng là trạng thái BÌNH THƯỜNG
      // khi nộp bài (lớp làm sâu bằng Gemini là tuỳ chọn). Client dựa vào mã này để hiển
      // thị nguyên dữ liệu nền từ eval/run-5 mà không coi là lỗi.
      json(res, 501, {
        error:
          'Chưa cấu hình GEMINI_API_KEY — bỏ qua bước làm sâu, dùng dữ liệu nền từ eval/run-5. ' +
          'Muốn bật: thêm GEMINI_API_KEY vào codebase/web/.env hoặc .env ở gốc repo.',
        disabled: true,
      })
      return
    }
