...
**Mức prototype:** Mock, dựng ở hai bản song song. **Bản 1 — `codebase/lecturefocus.html`** (HTML/CSS/JS thuần, dữ liệu nhúng cứng): dùng để chốt lát cắt và thiết kế 3 màn hình (Lesson → Review Map → Concept Detail), giữ nguyên làm bản tham chiếu, không phát triển thêm sau CP4. **Bản 2 — `codebase/webapp/`** (client + server thật, có deploy): dựng sau CP4 để phục vụ vòng validation với willing user (§8) — cùng một lát cắt và luồng, UI fetch dữ liệu qua API từ server thay vì đọc biến JS nhúng sẵn. Server có hai đường: (a) `GET /api/lessons/:id` serve lại dữ liệu tĩnh đã có (từ `eval/run-2/ai-output.json`) cho 2 bài học catalog; (b) `POST /api/generate` — **gọi AI thật lúc runtime**, dùng cho màn "Demo trực tiếp": chọn 1 trong 6 transcript của data pack (không nhận transcript tự do từ client, tránh tốn quota Groq ngoài kiểm soát), chạy đúng logic ở `codebase/lib/reviewMapGenerator.mjs` (dùng chung với `codebase/generate-review-map.mjs`), trả kết quả trực tiếp — không cache, mỗi lần bấm là một lượt gọi AI mới. Dựng đường (b) để đáp ứng CP6 §04-rubric.md: "giám khảo chạy 1 case lạ tại chỗ" trong vòng Q&A. Không RAG, không chatbot đa lượt — vẫn đúng lát cắt một quyết định AI duy nhất (transcript → Review Map), chỉ giờ có thể chạy trên bất kỳ transcript nào trong data pack thay vì chỉ Day 1. Trước CP3 đã có ít nhất một lời gọi AI thật ở đúng quyết định trung tâm — nhận transcript, sinh ra danh sách khái niệm kèm tier, lý do và trích dẫn — rồi so kết quả với golden set. Những phần vẫn mock ở cả hai bản: chọn nhiều bài học ở màn Home, nút "Hỏi AI Tutor", nghe lại audio, lưu tiến độ học viên.
...
| 2026-07-31 | Thêm `POST /api/generate` trong `codebase/webapp/server` — gọi AI thật (Groq) lúc runtime cho 1 trong 6 transcript của data pack, tách logic gọi AI ra `codebase/lib/reviewMapGenerator.mjs` dùng chung với `codebase/generate-review-map.mjs` | CP6 (`04-rubric.md`) có vòng Q&A "giám khảo chạy 1 case lạ tại chỗ" — bản chỉ serve dữ liệu tĩnh (run-2) không đáp ứng được yêu cầu này. Giới hạn chỉ nhận transcript có sẵn trong data pack (không nhận text tự do từ client) để kiểm soát chi phí/token Groq — free tier đã từng hết quota ngày 2026-07-30 (xem `eval/run-3/trace.json`) |
