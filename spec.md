...
**Mức prototype:** Mock, dựng ở hai bản song song. **Bản 1 — `codebase/lecturefocus.html`** (HTML/CSS/JS thuần, dữ liệu nhúng cứng): dùng để chốt lát cắt và thiết kế 3 màn hình (Lesson → Review Map → Concept Detail), giữ nguyên làm bản tham chiếu, không phát triển thêm sau CP4. **Bản 2 — `codebase/webapp/`** (client + server thật, có deploy): dựng sau CP4 để phục vụ vòng validation với willing user (§8) — cùng một lát cắt và luồng, nhưng UI fetch dữ liệu qua API từ server thay vì đọc biến JS nhúng sẵn. Server chỉ đọc/serve lại đúng dữ liệu tĩnh đã có (từ `eval/run-2/ai-output.json` + transcript), không tự gọi AI lúc runtime, không RAG, không chatbot — vẫn đúng lát cắt một quyết định AI đã khai, chỉ đổi cách UI lấy dữ liệu (embedded → fetch qua API) để có link deploy cho user thật bấm thử. Trước CP3 đã có ít nhất một lời gọi AI thật ở đúng quyết định trung tâm — nhận transcript, sinh ra danh sách khái niệm kèm tier, lý do và trích dẫn — rồi so kết quả với golden set (`codebase/generate-review-map.mjs`). Những phần vẫn mock ở cả hai bản: chọn nhiều bài học, nút "Hỏi AI Tutor", nghe lại audio, lưu tiến độ học viên.
...
| 2026-07-31 | Nâng cấp Prompt và Quick Prompts cho AI Tutor | Tránh phản hồi nông cạn và chung chung; tăng độ sâu học thuật, bổ sung ví dụ minh họa và trích dẫn cụ thể. Tích hợp cơ chế tự động xoay quanh tài liệu/tệp tải lên khi người dùng upload file. |
