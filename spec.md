...
**Mức prototype:** Mock. Ba màn hình bấm được (Lesson → Review Map → Concept Detail) chạy trên `codebase/lecturefocus.html` (HTML/CSS/JS thuần, không framework, không backend), dữ liệu nhúng cứng ngay trong file — đọc từ đúng output AI thật đã chạy (`eval/run-2/ai-output.json`), không phải dựng tay. Trước CP3 đã có ít nhất một lời gọi AI thật ở đúng quyết định trung tâm — nhận transcript, sinh ra danh sách khái niệm kèm tier, lý do và trích dẫn — rồi so kết quả với golden set (`codebase/generate-review-map.mjs`). Những phần vẫn mock: chọn nhiều bài học, nút "Hỏi AI Tutor", nghe lại audio, lưu tiến độ học viên.
