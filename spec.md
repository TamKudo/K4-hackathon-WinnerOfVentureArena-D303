...
**Bốn đường đi nằm ở đâu trong prototype:**

| Đường đi | Vị trí cụ thể trong `codebase/web/` |
|---|---|
| **Happy** | `pages/StudyHubPage.tsx` — bản đồ ba tầng Trọng tâm / Quan trọng / Bổ trợ, mỗi thẻ có tóm tắt + ước lượng thời gian; bấm vào mở `pages/ConceptDetailPage.tsx` với trích dẫn nguyên văn kèm `segmentId` |
| **Low-confidence** | `components/map/BentoConceptTile.tsx` — nhãn cam **"⚠ Tín hiệu chưa chắc"** cạnh tên khái niệm; `pages/ConceptDetailPage.tsx` — khối cảnh báo giải thích vì sao (đoạn nghe không rõ / giảng viên tự nhận xét chỉ mang tính tương đối) và nhắc đối chiếu lại trích dẫn. Cả hai đọc từ field `uncertain_signal` do **AI tự gán** ở `codebase/lib/reviewMapGenerator.mjs` quy tắc 3 — không phải nhóm gắn tay. *Ở lượt 5, AI không gắn cờ cho khái niệm nào (0/21), nên đường đi này không xuất hiện trên dữ liệu đang demo; đã kiểm chứng bằng cách bơm cờ vào dữ liệu test — xem ảnh trong `validation/` khi demo.* |
| **Failure** | `pages/StudyHubPage.tsx` — khi `list.length === 0`, hiện khối **"Chưa dựng được bản đồ ôn tập cho buổi này"** với lý do "không có khái niệm nào đủ căn cứ trong transcript... hệ thống không đoán liều", thay vì hiện danh sách rỗng hoặc bịa nội dung. Tương ứng nhánh trả `concepts: []` trong `reviewMapGenerator.mjs` |
| **Correction** | `pages/StudyHubPage.tsx` — bộ lọc quỹ thời gian 15/30/60 phút (`lib/reviewPlan.ts → planForBudget`); học viên đổi quỹ để rút gọn danh sách, hoặc bỏ qua khái niệm không đồng ý. Lọc bằng code phía client, **không gọi lại AI** |
...
| 2026-07-31 | **Từ feedback validation:** làm rõ tiêu chí xếp tier ngay trên màn Ôn tập, không bắt học viên mở từng khái niệm mới thấy lý do | **2/5 người thử phát hiện độc lập.** Hoàng Hải Dương (phiên #3) nói thẳng *"không hiểu tiêu chí trọng tâm"*; Nguyễn Yến Khanh (phiên #5) quan sát thấy không hiểu ngay vì sao concept được xếp Trọng tâm. Đây là lỗi mức Cao — người dùng thấy nhãn Trọng tâm/Quan trọng/Bổ trợ mà không biết dựa trên căn cứ gì, làm hỏng đúng nguyên tắc G2 (làm rõ mức độ tin cậy) và G11 (giải thích vì sao) đã khai ở §4. Lý do hiện chỉ nằm trong khối "Vì sao xếp trọng tâm?" ở trang chi tiết — quá sâu. Chi tiết: `validation/feedback-log.md` |
| 2026-07-31 | **Từ feedback validation:** làm phần trích dẫn (evidence) trông bấm được rõ hơn | Nguyễn Yến Khanh (phiên #5) mất **~20 giây** mới nhận ra trích dẫn có thể click để mở transcript. Đây là tính năng lõi — nếu không phát hiện ra thì học viên mất luôn khả năng tự kiểm chứng, đúng thứ phân biệt LectureFocus với việc dán tài liệu vào ChatGPT (§3). Gạch chân hiện tại chưa đủ báo hiệu affordance |
| 2026-07-31 | **Từ feedback validation — quyết định GIỮ NGUYÊN non-goal quiz** dù có người đề nghị | Cùng phiên #3 đề nghị thêm "quiz nhỏ". Giữ nguyên vì có căn cứ số: khảo sát 20 người cho thấy chỉ **10% chọn quiz** khi có 15 phút trước Lab/Quiz, trong khi **90% chọn "trích xuất top 3 kiến thức cốt lõi kèm trích dẫn"**. Thêm quiz sẽ phá lát cắt một quyết định AI và mâu thuẫn kịch bản §5 số 5. Tương tự, đề nghị map khái niệm sang **slide** (phiên #1) cũng giữ nguyên: data pack chỉ có transcript, không có slide — tự suy diễn sẽ vi phạm nguyên tắc mọi khái niệm phải có trích dẫn xác minh được. Cả hai đưa vào backlog slide 6 |
