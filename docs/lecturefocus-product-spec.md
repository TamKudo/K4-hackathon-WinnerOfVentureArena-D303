...
`DATA.lessons["day01-foundation"].concepts` không còn là dữ liệu dựng tay (lượt 0) — đã nối trực tiếp với `eval/run-2/ai-output.json` bằng `codebase/wire-ai-output-to-ui.mjs` (`node codebase/wire-ai-output-to-ui.mjs [đường dẫn ai-output.json]`, mặc định run-2). Vì quality bar (≥70%) **chưa đạt** ở lượt 2 (59,4%), UI đang hiển thị đúng kết quả thật kèm hạn chế đã ghi nhận trong `eval/run-2-results.md` (ví dụ: thiếu "Quản lý Context/temperature" ở tier phù hợp, bỏ sót 1/4 cuối transcript) — đúng luật "kết quả thấp không ảnh hưởng điểm nếu ghi nhận đầy đủ, trung thực". Lượt 3 đã thử (pipeline sửa xong, xem `spec.md` §7) nhưng bị Groq chặn hết quota token/ngày ngay từ lô 1 — không có output để nối. Nếu quota hồi và chạy được trước CP6, lặp lại đúng phép nối này với `eval/run-N/ai-output.json` của lượt mới nhất đạt bar cao hơn.